# SCM Graph Proposed API 實作計畫

> `Agentic worker requirement`：使用 `executing-plans`，並依序完成每個 TDD checkpoint。
...
### Task 1：Manifest regression contract（Manifest 回歸契約）
...
- Produces：有效的 `package.json#enabledApiProposals` 與 `package.json#contributes.menus.scm/historyItem/context`。
...
### Task 2：Development-host launch contract（Development Host 啟動契約）
...
### Task 3：文件與完整驗證 (Documentation and Full Verification)
