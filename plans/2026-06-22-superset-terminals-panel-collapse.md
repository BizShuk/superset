...
`vscode.Terminal` 物件無法 JSON 序列化 → webview ↔ host 用「穩定字串 ID」指認。**策略**:`registry` 內每個 `Entry` 帶 `id: string`,在 `add()` 時用 `terminal.processId?.toString() ?? \`t-${Date.now()}\`` 當 ID;close 時 `registry.remove` 同步刪除 entry,ID 隨之釋放,沒有 stale 風險。
...
| `media/panel.html` | **新增** | webview 入口 HTML (CSP,inline script / panel.js) |
