...
方向命名以`群組排列方向`為準，與 VS Code 選單的`分割方向`用語相反 —— 兩者不可混用：

| 本專案用語   | 群組排列    | `orientation` | VS Code 選單用語         |
| ------------ | ----------- | ------------- | ------------------------ |
| `Horizontal` | 左 → 右(欄) | `0`           | Vertical Editor Layout   |
| `Vertical`   | 上 → 下(列) | `1`           | Horizontal Editor Layout |
...
| `Set Total`             | 一個 sibling set 現有的像素總量；重寫比例時沿用它，取不到才退回 `FALLBACK_SET_TOTAL`。                                            |
| `Grid Appearance Order` | VS Code `getGroups(GRID_APPEARANCE)` 的深度優先順序，等同 descriptor 的 leaf 序，也等同 `viewColumn - 1`。                        |
| `Creation Order`        | `tabGroups.all` 的順序(group 建立先後)。與 Grid Appearance Order 不同，不可用於 layout indexing。                                 |
| `Signature Guard`       | 比對本次與上次`寫入`的 descriptor，阻斷 `setEditorLayout` 自身事件造成的無限重套；不與即時佈局比對。                              |
...
| `Default Tools`                 | `pm2`、`skills`、`dux`、`port`、`sessiond`、`autop`、`auth`、`proxy`、`mdserver` 與 `ytdl` 十個 CLI；各自在獨立 run terminal 以 `go install ...@master` 安裝。                                                                                                    |
...
| `Extension Settings`            | `Superset: Open Settings` 開啟的 VS Code native Settings，query 固定過濾 `@ext:shuk.superset`。                                                                                                                                                                  |
| `Diagnostics Snapshot`          | `Superset: Show Diagnostics` 從 active plugin providers 即時計算的一次性 Markdown snapshot；包含 active plugin IDs 與 terminal、mDNS、TODO counts。                                                                                                             |
...
| 術語                          | 定義                                                                                                                              |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `Composition Root`            | `src/extension.ts`；建立 root-owned capabilities 並宣告 plugin activation order。                                                 |
| `Feature`                     | 一個使用者或 domain 能力，程式放在 `src/<feature>/`。                                                                             |
| `Plugin`                      | 實作 `ExtensionPlugin`、以 `PluginContext` 啟用一個 Feature 的 lifecycle unit。                                                   |
| `Plugin Manager`              | 依序 activate/deactivate plugins、隔離錯誤、管理 disposables/reset handlers/diagnostics providers，並組合 Markdown contributors。  |
| `Plugin Context`              | Composition Root 注入 plugin 的受控 dependencies，包括 workspace、state、log、native terminal、Tree View 與 runtime hooks。       |
| `Activation Order`            | Plugin 啟用順序；Markdown composition、command availability 與 panel layout restore 依賴此順序。                                  |
| `Error Isolation`             | 單一 plugin activation failure 會清除其 partial resources，但不阻止 sibling plugins activation。                                  |
...
| `Diagnostics Provider`        | Plugin 註冊的 read-only live metric callback；snapshot 時才執行，單一 provider 失敗不影響其他 providers。                          |
