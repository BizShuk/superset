...
| `Skill Repository`             | 傳給 `skills add` 的 GitHub identifier。Quick Pick 清單與順序由 `src/installCommands.ts#SKILL_REPOSITORIES` 定義；`description` 顯示用途，`detail` 顯示 GitHub repository，`bizshuk/cc-plugin` 為預設。 |
| `Projects Setup`               | `Superset: Projects Setup`；建立固定 `~/projects` root，clone 標準 BizShuk repository set 並初始化 recursive submodules。     |
| `Projects Setup Repository Set` | `env_setup`、`cc-plugin`、`ai`、`game`、`data`、`iphone`、`platform`、`playground`、`product`、`research`、`tools`、`web`。   |
