...
同一台主機以多個 mDNS 實例名稱廣播、或同時走多張網卡 / IPv4+IPv6 時,自動以 `host|port|type` 為網路身分去重,合併成同一列;其餘名稱顯示在該列的「別名」(aliases)細節欄位。

#### 2.2 過期移除

服務一段時間未再廣播即自動過期移除:以 `3 × TTL` 為寬限期 (RFC 6762),沒帶 TTL 的記錄 fallback 到 120 秒。
...
**功能**:在 Activity Bar 的第二顆 `Superset-Overall` icon 下,總覽 `~/projects/` 與 `~/projects/tmp/` 兩個根目錄第一層子目錄裡,**所有含 `README.todo` 的專案**。每個 project row 一律預設收合,展開可看:
...
2. 面板列出 `~/projects/` 與 `~/projects/tmp/` 下所有有 `README.todo` 的專案 row(預設全部收合)。
...
2. 按 `F5` → 跳出 `Extension Development Host` 視窗(已載入本擴充)。
3. 在新視窗驗證功能。
