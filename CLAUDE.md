...
- CLI Launcher 的掃描條件只有「是不是可列出的目錄」，不得按內容篩選（例如只留有 `.git` 的資料夾）：面板要挑的 cwd 不限於 repository，而按內容篩選會讓「東西不見了」變成沒有出口的狀態。撈到不想要的資料夾一律靠 `hidden` 手動移除。
